...
export async function exportToPDF(title: string, columns: string[], data: (string | number)[][], fileName: string) {
  ...
  // Encabezado institucional — mismos colores que la Hoja de Calificación: negro y negrita, sin azul.
  doc.setFontSize(13);
  doc.setTextColor(0, 0, 0);
  doc.setFont('helvetica', 'bold');
  doc.text('UICYT · UNIDAD DE INVESTIGACIÓN CIENCIA Y TECNOLOGÍA', 14, 15);
  ...
  doc.text(`Generado el: ${new Date().toLocaleString('es-ES', { dateStyle: 'long', timeStyle: 'short' })}`, 14, 28);
  ...
  autoTable(doc, {
    ...
    body: data,
    theme: 'grid',
    styles: { lineColor: [30, 41, 59], lineWidth: 0.15 },
    headStyles: {
      fillColor: '#d9d9d9',
      textColor: '#1e293b',
      fontStyle: 'bold',
...
// ─── HOJA DE CALIFICACIÓN — RESPALDO FÍSICO DE EVALUACIÓN POR JURADO ────────
// Reproduce la grilla oficial (columnas ASPECTO / CRITERIO / INDICADORES +
// escala), ya llenada con los puntajes y observaciones de cada evaluación
// confirmada, para imprimir y hacer firmar al jurado como respaldo físico.
