...
export default function EvaluarProyectoVisitante() {
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      // Bloqueo: si ya evaluó, redirige a completado
      const yaEvaluo = localStorage.getItem(`visitante_eval_${user.id}_${id}`);
      if (yaEvaluo) {
        router.replace('/visitante/completado1');
        return;
      }
  ...
  // Solo localStorage, nunca Supabase
  const handleConfirmar = () => {
    if (!idUsuario) return;
    setSaving(true);
    localStorage.setItem(`visitante_eval_${idUsuario}_${id}`, 'true');
    router.push('/visitante/completado1');
  };

  if (loading) return (
    <LoadingScreen
      message="Cargando proyecto a evaluar..."
      submessage="Instrumento de Evaluación · UICYT"
      fullScreen={true}
    />
  );
  ...
  if (step === 1) return (
    <div style={{ minHeight: '100vh', background: '#f8fafc' }}>
      <AppHeader onBack={() => router.push('/visitante')} />
